...
BOOL CCommDlg::OnInitDialog()
{
	...
	((CButton *)GetDlgItem(IDC_CHECK_SEND2))->SetCheck(FALSE);
	...
	//指令初始化
	SetDlgItemText(IDC_COMBO_Order1, _T("读取信息"));
	...
	SetDlgItemText(IDC_COMBO_Order6, _T("修改编号"));
...
void CCommDlg::OnClickedButtonSend1()
{
	...
	else if(IsDlgButtonChecked(IDC_CHECK_SEND2))//按钮被按下
	{
		SetTimer(1,500,NULL);
	}
	else
