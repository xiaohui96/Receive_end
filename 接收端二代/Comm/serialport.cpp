...
BOOL SerialSend(BYTE* buffer, WORD length)
{ 
...
{  
    ...
    TimeOuts.ReadTotalTimeoutMultiplier = 10;  //读取一个字节和等待下一个字节所需时间
    TimeOuts.ReadTotalTimeoutConstant = 500;   //一次读取串口数据的固定超时
...
DWORD sSerialRecv(BYTE* buffer, DWORD bufMaxSize)
{
	...
	{
		//进入这里表示未读入任何字节
		dwRes = WaitForSingleObject(Rol.hEvent, 500);   //5秒超时
		switch(dwRes)
		{
			case   WAIT_OBJECT_0:
				if (!GetOverlappedResult(hCom,
